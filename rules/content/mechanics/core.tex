...
Another way to visualise the outcome of a 4dF is as the chance you have of rolling higher than some threshold value, here shown for thresholds -5 (where the success probability is 100\% if the smallest value you can roll is -4) to +4 (the highest you can roll with a 4dF):
...
\begin{DndTable}[header=Difficulty Levels in \emph{The Wyrd Engine}]{lX}
    ...
    \hline
    \textbf{Trivial (-2)} & A task so easy that failure is nearly impossible (walking across a stable floor, recalling your own name). \\
    \textbf{Simple (-1)} & A straightforward action requiring minimal effort (identifying a common herb, climbing a ladder). \\
    \textbf{Basic (0)} & An ordinary action requiring little skill (jumping over a puddle, recalling well-known facts). \\
    \textbf{Challenging (+1)} & A moderate test of skill or effort (spotting a hidden compartment, climbing a wooden fence). \\
    \textbf{Difficult (+2)} & A task requiring training or experience (tracking footprints in the rain, persuading a sceptical guard). \\
    \textbf{Formidable (+3)} & A demanding task that pushes a character’s limits (picking a complex lock under pressure, leaping between rooftops). \\
    \textbf{Arduous (+4)} & A near-impossible feat requiring mastery (detecting a forged document at a glance, sniping a target from extreme range). \\
    \textbf{Extreme (+5)} & A task bordering on the impossible (convincing a lifelong enemy to trust you, performing surgery in total darkness). \\
\end{DndTable}

For levels up to \textbf{Basic}, rolls are usually unnecessary unless dramatic tension is involved. For characters with appropriate skills, \textbf{Basic} tasks can also be handled without rolls.
...
The graph tells us that even trivial tasks can fail if you are unskilled and unlucky enough, and routine tasks will fail a third of the time for someone without the necessary skills. Adding skills effectively shifts the difficulty levels. When we add +1 to a roll, the unmodified range -4 to +4 is shifted to -3 to +5, for example. Adding one to the result of the dice roll is equivalent to subtracting one from the difficulty level. So, to see how difficult a task is for someone with a skill level of +1, you shift the difficulty levels down by one. To see how difficult a task is with skill levels +2 a +3, you shift by two and three. So a \textbf{Challenging} task for an untrained character (with a 19\% chance of success) will effectively only be \textbf{Basic} for a character with skill level +1.
...
\begin{DndTable}[header=Success probability per skill level]{lrrrr}
    \textbf{Difficulty Rating} & \textbf{Skill 0} & \textbf{Skill +1} & \textbf{Skill +2} & \textbf{Skill +3}\\
    \hline
    \textbf{Trivial (-2)}     & 82\% & 94\% & 99\% & 100\% \\
    \textbf{Simple (-1)}      & 62\% & 82\% & 94\% & 99\% \\
    \textbf{Basic (0)}        & 38\% & 62\% & 82\% & 94\% \\
    \textbf{Challenging (+1)} & 19\% & 38\% & 62\% & 82\% \\
    \textbf{Difficult (+2)}   &  6\% & 19\% & 38\% & 62\% \\
    \textbf{Formidable (+3)}  &  1\% &  6\% & 19\% & 38\% \\
    \textbf{Arduous (+4)}     &  -   &  1\% &  6\% & 19\% \\
    \textbf{Extreme (+5)}     &  -   &  -   &  1\% &  6\% \\
\end{DndTable}
